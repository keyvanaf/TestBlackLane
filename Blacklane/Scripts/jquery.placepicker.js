+(function ( $, window) {
  ...
  function PlacePicker(element, options) {
    ...
    function template() {
        ...
        "<div class=\"input-group \">" +
          "<span class=\"input-group-btn \">" +
            "<button type=\"button\" data-toggle=\"modal\" href=\"#" + options.mapContainerId + "\" class=\"btn btn-default \">" +
              "<span class=\"" + options.mapIconClass + "\"></span>" +
            "</button>" +
            "</span>" +
        "</div>" +
        '<div id=\"' + options.mapContainerId + '\" class="modal fade" role="dialog">                              \
            <div class="modal-dialog" >                                                                            \
                <div class="modal-content">                                                                        \
                    <div class="modal-header">                                                                     \
                        <button type="button" class="close" data-dismiss="modal" title="Map">&times;</button>      \
                        <h4 class="modal-title">'+ element.title+'</h4><span class="titleLocation"></span>         \
                    </div>                                                                                         \
    ...
    function initMap() {
      ...
      var titleLocation = $("#" + options.mapContainerId + " .titleLocation");
      ...
      // refreshed.
      $("#" + options.mapContainerId).on("show.bs.modal", function (e) {
          $(e.target)
              .css("display", "block")
              // Fix map icons
          ...
          if (!element.value) {
              ...
              //bug:don't show map for first time
              //solve:resize map async
              setTimeout(function () {
                  instance.resize();
              },200);
          } else {
          ...
          clone = $(element).appendTo(titleLocation).clone().appendTo(parent);
          $(e.target).css("display", "");
      });
      $("#" + options.mapContainerId).on("hide.bs.modal", function (e) {
